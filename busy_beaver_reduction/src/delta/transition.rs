...
impl Transition {
    ...
    /// Given a transition in the form of a `Vec<u8>`,
    /// transforms all u8 elements into `String` and concatenates
    /// them using a `comma`.
    ///
    ...
    pub fn encode(transition: Vec<u8>) -> String {
        return transition
            .iter()
            .map(|&u| u.to_string())
            .collect::<Vec<String>>()
    ...
    /// Given an entry in the hashmap of transitions, transforms
    ...
    pub fn encode_from_hashmap(transition: (&(u8, u8), &(u8, u8, Direction))) -> String {
        return Transition::encode(Transition::get_from_hashmap(transition).get_as_vec());
    }

    /// Given a `String`, reconstructs the self `Transition` attributes
    /// after decoding it.
...
mod tests {
    ...
    fn test_encode() {
        ...
        assert_eq!(Transition::encode(transition.get_as_vec()), "0,0,1,1,1");
    }

    #[test]
    fn test_encode_from_hashmap() {
        let transition: (&(u8, u8), &(u8, u8, Direction)) = (&(0, 0), &(1, 1, Direction::RIGHT));
        assert_eq!(Transition::encode_from_hashmap(transition), "0,0,1,1,1");
    }
